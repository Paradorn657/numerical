...
const Newton = () => {
    ...
    const calonepoint = (xstart, er) => {
        ...
        let x0 = xstart;
        let f_x0 = evaluate(Equation,{x:x0});
        let f_px0 = derivative(Equation, 'x').evaluate({x:x0})
        
        let xold = 0;
        let xn = x0 + ((-1 * f_x0) / f_px0);
        ...
        while (Math.abs((xn - xold) / xn) * 100 > er) {
          ...
          let f_x = evaluate(Equation,{x:xn});
          let f_px = derivative(Equation, 'x').evaluate({x:xn})
