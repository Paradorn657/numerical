    ...
    const predict_value = () => {
        ...
        if (splineType == "Linear") {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                for (let j = 0; j < 2; j++) {
                    if (j !== i) {
                        term *= (xdata[j] - x) / (xdata[j] - xdata[i]);
                    }
                }
                result += term;
            }
            ...
            for (let i = 0; i < usepoint.length; i++) {
                let term = yData[i];
                for (let j = 0; j < usepoint.length; j++) {
                    if (j !== i) {
                        term *= (xdata[j] - x) / (xdata[j] - xdata[i]);
                    }
                }
...
export default Lagrance_interpolation;
